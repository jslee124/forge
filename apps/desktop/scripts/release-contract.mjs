...
for (const arch of windows ? ["x64"] : ["arm64", "x64"]) {
  for (const extension of windows ? ["exe"] : ["dmg", "zip"]) {
...
await writeFile(
  ...
  `${JSON.stringify({ tag, version, packageVersion: pkg.version, platform: windows ? "win32" : "darwin", channel: version.split("+")[0].includes("-") ? "preview" : "stable", assets }, null, 2)}\n`,
);
console.log(
