...
日期：2026-09-25。状态：**待用户确认的开发方案**；以下是拟实施行为，不代表当前产品已具备。此轮只编写文档。
...
![macOS 任务侧栏折叠后，红绿灯仍固定在窗口左上角](design/refinement-2026-09-25/workbench-collapsed-macos-reference.png)

这三张图片用于确认视觉方向和布局层级，不是应用截图或可执行原型。图片中的示例状态、辅助文案、任务列表、按钮细节与实际产品数据应以本方案的交互要求和当前源码为准；实现前还需分别检查 Windows 布局、键盘操作和窄窗口。
...
**macOS：**使用 Electron `titleBarStyle: "hiddenInset"` 保留系统原生红绿灯，不绘制假的红黄绿按钮。让窗口内容背景延伸至顶部，红绿灯占据侧栏顶行左侧，铁砧/Forge 品牌位于其右；侧栏折叠按钮移到不与红绿灯争空间的位置。顶部可拖动空白区、所有交互控件为不可拖动区；全屏、缩放、双击标题区、浅深色和窄窗口均应保留系统预期行为。先用真实 macOS 窗口截图检查坐标，再决定是否需要 `trafficLightPosition` 微调。

**macOS 折叠态：**红绿灯的窗口坐标保持不变，不随侧栏宽度移动或缩小；只有侧栏内容折叠。顶部预留独立的约 44–52 px 原生控件安全区，折叠栏计划宽约 88–96 px（最终以真实系统控件外框和命中区实测确定），不能沿用当前 64 px。隐藏 FORGE 字标和任务列表，将可点击、可键盘聚焦的铁砧“展开侧栏”按钮置于红绿灯**下方**，新任务与设置图标再依次向下排。折叠态不显示调宽手柄；重新展开恢复上次宽度。系统全屏模式中红绿灯的显隐遵循 macOS 原生行为，不强制绘制替代控件。
...
使用网格列宽的单一状态作为布局来源，去掉 `.app-frame` 网格宽度与 `.sidebar` 自身宽度不一致的固定值。展开侧栏默认约 224 px；拖动边界时即时更新，宽度限制为约 160–360 px，同时保证主内容至少约 520 px。窗口缩小时只临时夹紧显示宽度，不覆写用户保存的偏好；扩大后恢复偏好宽度。折叠宽度按平台定义：macOS 约 88–96 px 以完整容纳固定红绿灯安全区，Windows 仍可用当前约 64 px 的紧凑栏；重新展开恢复上次展开宽度。
...
- macOS 顶部红绿灯方案必须在最小和最大侧栏宽度下都不与品牌、折叠按钮或拖动区域重叠；Windows 原生标题栏布局独立检查。
- 增加展开→折叠→展开的真实窗口截图和点击验收：红绿灯保持固定、三颗按钮都能点击，铁砧展开按钮不误触窗口控件，主内容利用释放的宽度；在最小窗口、全屏和设置页重复检查。
...
请先确认这个方向，尤其是：新默认模型采用 `deepseek-flash`、旧 ID 只做兼容；选择模型后关闭弹层；macOS 将原生红绿灯融入侧栏顶行而 Windows 保留原生标题栏；设置页使用四分区导航；任务侧栏支持拖动、键盘调整和记住宽度。确认前不修改运行时代码、样式或图标资源。
