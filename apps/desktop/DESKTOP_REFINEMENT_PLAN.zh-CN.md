...
- 当前源码已有 `deepseek-flash` 默认值与旧 ID 兼容处理、双平台铁砧图标、四分区设置页、模型选择器的选择/外部点击关闭，以及任务侧栏的拖动、键盘调宽和宽度记忆。本轮没有重新声明这些能力的已安装应用验收。
- 本轮将 macOS 折叠态改为完全隐藏侧栏，在原生红绿灯右侧的主内容顶部放置展开侧栏、工作空间和新任务入口，右侧保留工作台和设置入口。展开态字标位于顶部控件下一行；Windows 仍使用原生标题栏和原有折叠布局。模型弹层现在按主内容宽度定位，避免右侧工作台打开时越入侧栏。
- `CI=true pnpm check`、`CI=true pnpm desktop:build`、本地 Electron 完整界面验收和更新设置页验收通过。离线 fixture 与隐藏窗口截图验证了折叠/恢复、入口、窄窗口和弹层边界；另在可见 macOS 开发窗口检查了顶部布局和系统窗口按钮的无障碍节点。屏幕共享提示覆盖了按钮截图，全屏截图也未能用于可靠视觉判断，因此红绿灯实际命中、系统全屏、Windows 窗口及已安装包外观仍需独立验收。

## 确认点

macOS 折叠侧栏方向已于 2026-09-26 选定，并在当前开发源码中实施。原方案的其他确认点包括：以 `deepseek-flash` 为新默认、旧 ID 保持兼容；选择模型后关闭弹层；设置页四分区导航；任务侧栏支持拖动、键盘调整和记住宽度。上述实施状态须以当前源码和独立验收核对，不能把这份设计记录当作已发布产品证据。
