...
保留规范 fixture，增加至少两个任务和 hidden grader；默认 suite 使用 fake model；付费 DeepSeek 试验显式 opt-in；多次运行记录 model/settings、通过率、耗时、步骤、工具调用和 token；加入 terminal demo、README setup/results/limitations、许可证、model ID 复核并打 `v0.1` tag。所有 v0.1 gate 见[历史 v0.1 验收合约](../v0.1/ACCEPTANCE.md)。
...
Resume 实现方案](../v0.3.3/STRUCTURED_SESSION_HISTORY.md)。
...
测试矩阵、rollout gate 与 rollback 见 [v0.3.4 详细实现方案](IMPLEMENTATION_PLAN.md)。
