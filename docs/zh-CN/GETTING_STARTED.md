...
最后一条命令会构建 workspace；当前源码 release 应输出 `0.3.0-bootstrap.0`。它不会联系模型 provider。
...
Session 保存已经完成的 user/assistant turns；run 是一次带独立 ID 和 JSONL event trace 的有界 agent-loop 执行。Resume 只恢复已完成对话文本，不恢复旧审批、待执行 tool call、子进程或 provider continuation state。详见[会话与 trace](SESSIONS.md)。
