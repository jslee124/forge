...
Forge 0.3.1 不依赖插件。插件是可选的进程内 JavaScript module，可以注册模型调用工具和显式本地命令、贡献指令、观察不可变 run event，或让策略更严格。实现中的类型、schema 和 host 才是最终合约：[types.ts](../../packages/plugin-api/src/types.ts)、[schema.ts](../../packages/plugin-api/src/schema.ts)、[host.ts](../../packages/plugin-api/src/host.ts)。
